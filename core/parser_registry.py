...
``prompt_profiler.tasks.<name>.parsers`` modules at package init time,
...
Usage (automatic — called from prompt_profiler/__init__.py):
    from prompt_profiler.core.parser_registry import autoload_parsers
    autoload_parsers()
...
# e.g. "prompt_profiler.tasks.wtq.parsers" -> {"code": <fn>, "answer": <fn>, ...}
...
def autoload_parsers() -> None:
    ...
    try:
        import prompt_profiler.tasks as tasks_pkg
    except ImportError:
        logger.warning("autoload_parsers: could not import prompt_profiler.tasks")
        return

    for _, name, is_pkg in pkgutil.iter_modules(tasks_pkg.__path__):
        if not is_pkg:
            continue  # only recurse into sub-packages
        module_path = f"prompt_profiler.tasks.{name}.parsers"
        try:
