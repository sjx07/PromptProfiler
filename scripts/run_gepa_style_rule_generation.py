#!/usr/bin/env python3
"""Small GEPA-style rule generation batches over FACET cube failures.

This is a discovery tool, not an experiment runner. It samples compact failure
batches from an existing cube, asks a local instruction model to reflect over
those failures, and writes structured candidate prompt rules for later FACET
validation.
"""
...
DEFAULT_OUT = Path("study_layer/artifacts/gepa_rule_generation_v0")
...
def system_prompt() -> str:
    return (
        "You imitate GEPA-style prompt reflection for data tasks. "
        "Given compact failed trajectories, infer reusable prompt rules. "
        "Rules must be small, concrete, and conditional. Avoid generic 'think step by step'. "
        "Do not propose output formatting fixes unless the cases are purely parser failures. "
        "Generalize one level beyond exact entities, but do not invent unsupported benchmark facts. "
...
def user_prompt(batch: dict[str, Any]) -> str:
    ...
    return (
        ...
        "Propose 3-6 candidate prompt rules that could rescue the failures with minimal off-slice harm.\n\n"
        ...
        "      \"rule_text\": one prompt-ready instruction sentence or short bullet,\n"
        "      \"context_trigger\": when to apply it,\n"
        ...
        "      \"expected_help\": why it helps these failures,\n"
...
def write_report(out_dir: Path, batches: list[dict[str, Any]], responses: list[dict[str, Any]], candidates: list[dict[str, Any]]) -> None:
    md_lines = [
        ...
        f"Candidate rules parsed: `{len(candidates)}`",
        "",
        "## Batches",
        "",
    ]
    ...
    for rule in candidates:
        rows_html.append(
            ...
            f"<td>{html.escape(str(rule.get('context_trigger', '')))}</td>"
            f"<td>{html.escape(str(rule.get('rule_text', '')))}</td>"
            f"<td>{html.escape(str(rule.get('validation_slice', '')))}</td>"
            "</tr>"
        )
    html_doc = f"""<!doctype html>
<html><head><meta charset=\"utf-8\"><title>GEPA-style Rule Generation v0</title>
<style>
...
<h1>GEPA-style Rule Generation v0</h1>
<p>Candidate rules are failure-driven hypotheses for later FACET validation.</p>
<p><b>Batches:</b> {len(batches)} &nbsp; <b>Candidate rules:</b> {len(candidates)}</p>
<table><thead><tr><th>Batch</th><th>Rule</th><th>Family</th><th>Risk</th><th>Trigger</th><th>Rule Text</th><th>Validation Slice</th></tr></thead>
